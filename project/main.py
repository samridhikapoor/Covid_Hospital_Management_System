...
app.config['SQLALCHEMY_DATABASE_URI']='mysql://root:@localhost/covid_database'
db=SQLAlchemy(app)
...
class Hospitaldata(db.Model):
    id=db.Column(db.Integer,primary_key=True)
    hcode=db.Column(db.String(20),unique=True) #hospital code
    hname=db.Column(db.String(100))
    normalbed=db.Column(db.Integer)
    hicubed=db.Column(db.Integer) #highicu 
    icubed=db.Column(db.Integer)
    vbed=db.Column(db.Integer) #ventilator bed

class Trig(db.Model): #display deatils
    id=db.Column(db.Integer,primary_key=True)
    hcode=db.Column(db.String(20))
    normalbed=db.Column(db.Integer)
...
class Bookingpatient(db.Model): #patient details
    id=db.Column(db.Integer,primary_key=True)
    srfid=db.Column(db.String(20),unique=True)
...
@app.route('/addHospitalUser',methods=['POST','GET'])  # admin registers a hospital 
def hospitalUser():
    ...
    if('user' in session and session['user']==params['user']):
        ...
        if request.method=="POST":
            ...
            mail.send_message('MEDIBAY HEALTHCARE CENTER',sender=params['gmail-user'],recipients=[email],body=f"Welcome User! Thanks for choosing MediBay\nAt MediBay, we place our patient's health and safety above all. During this coronavirus (COVID-19) health emergency, we're doing everything we can to serve our communities while also protecting our patients and answering your questions.\n\nYour Login Credentials Are:\n Email Address: {email}\nPassword: {password}\n\nHospital Code {hcode}\n\n Do not share your password\n\n\nThank You and best wishes\nTeam MediBay" )
...
@app.route("/addhospitalinfo",methods=['POST','GET'])   # hospital enters info
def addhospitalinfo():
...
@app.route("/hedit/<string:id>",methods=['POST','GET'])  # update hospital data
@login_required
def hedit(id):
...
@app.route("/hdelete/<string:id>",methods=['POST','GET'])  # delete hospital data
@login_required
def hdelete(id):
...
@app.route("/slotbooking",methods=['POST','GET'])  #patient enters his personal information
@login_required
def slotbooking():
